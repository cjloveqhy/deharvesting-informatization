...
import {getDispatchIds} from "@/api/uavHarvestSchedule";
...
let getHsIdUavHarvestSchedule = ref<GetHsIdUavHarvestScheduleFo>({
  belonger: route.params.belonger as string,
  cottonFieldId: route.params.cottonFieldId as string
})

let cottonFieldInfo = ref();

function getDispatchId(){
  getDispatchIds(getHsIdUavHarvestSchedule.value).then(res => {
    dispatchId.value = res.data
    getData()
  })
}
function getData() {
  loading.value = true
  getDetails(dispatchId.value as string).then(res => {
    dispatchDetails.value = res.data
    dispatchDetails.value.children.forEach(item => {
      if (item.id == route.params.cottonFieldId){
        cottonFieldInfo.value = item
      }
    })
...
const back = () => router.push({name: 'jobEvaluation_check'})
          ...
          <n-flex vertical size="large">
            <n-flex :wrap="false">
              <n-form-item class="w-1/3" label="地块名称：">
                {{ cottonFieldInfo.plotName }}
              </n-form-item>
              <n-form-item class="w-1/3" label="地址：">
                {{ cottonFieldInfo.addr }}
              </n-form-item>
              <n-form-item class="w-1/3" label="联系人：">
                {{ cottonFieldInfo.contacts.username }}
              </n-form-item>
            </n-flex>
            <n-flex :wrap="false">
              <n-form-item class="w-1/3" label="联系方式：">
                {{ cottonFieldInfo.contacts.phone }}
              </n-form-item>
              <n-form-item class="w-1/3" label="种植面积（亩）：">
                {{ cottonFieldInfo.cultivatedArea }}
              </n-form-item>
              <n-form-item class="w-1/3" label="棉花品种：">
                {{ cottonFieldInfo.cottonVariety }}
              </n-form-item>
            </n-flex>
          </n-flex>
        </n-form>
